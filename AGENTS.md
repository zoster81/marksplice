...
M0 is the green repository-bootstrap baseline and M1–M117 are complete engineering milestones. The native-parser replacement roadmap is complete at M115. The approved pre-M116 performance campaign is released as `v0.5.0-beta.1`: all six measured throughput/allocation gates plus corpus/conformance/fuzz/race/static/security/cross-platform gates are green, without adding product surface or reintroducing Goldmark. M116–M117 add the unreleased read-only `workspacefs` foundation plus hardened filesystem relationship resolution over caller-supplied `fs.FS` authority; they are not part of the v0.5 release. M118 is the next engineering boundary, followed by M119–M124 toward `v1.0.0`, with M125–M126 queued for the `v1.5.0` PDF line. `docs/roadmap.md` owns that sequence. Detailed completed-milestone contracts and historical evidence live only in the milestone records; current product capability belongs in `docs/capabilities.md`, while published-release state belongs in `CHANGELOG.md`/release documentation.
