...
- Goldmark is the current temporary semantic parser implementation and must remain an implementation detail until its mandatory M115 removal. It is not a normative source and differential mismatches are investigation signals, not automatic proof that native behavior is wrong. M111 freezes the complete substitution boundary as `internal/parser.Backend`; M112 and M113 add native block, inline, and parsed-document relationship candidates under `internal/parser/native`, while production `internal/splice` still reaches Goldmark through one explicit bridge. M114 owns specification-first recertification and native-backend hardening before the explicit M115 cutover. Never expose Goldmark AST nodes or Goldmark-specific types from Marksplice public APIs.
...
M0 is the green repository-bootstrap baseline and M1–M114 are complete engineering milestones. The current roadmap boundary is M115 Goldmark removal and final cutover. Detailed milestone contracts and historical evidence live only in the milestone records; current product status belongs in `docs/capabilities.md`.
