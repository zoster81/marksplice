...
The repository has a green retrospective M0 bootstrap record and completed engineering milestones M1–M103. The current model has two deliberately separate paths:
...
Executable examples for construction, parsing, bounded `QueryNodes` selection, link intelligence, explicit multi-document graph construction, workspace validation, GitHub alert recognition, fenced-block metadata reading, source-preserving heading mutation, and atomic `ComposeChanges` mutation composition live in `example_test.go` and are published by pkg.go.dev.
