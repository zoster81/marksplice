...
- **Export deterministic canonical Markdown explicitly.** `RenderCanonicalMarkdown` streams one stable semantic representation and `CanonicalMarkdown` returns caller-owned bytes; neither API mutates the parsed source or replaces source-preserving edits.
...
The document core does not crawl files or fetch URLs. `workspacefs` performs only explicit read-only Markdown discovery/following inside the `fs.FS` authority supplied by the caller. Rendering is explicit in-memory/output-writer work: canonical Markdown deliberately normalizes only its returned export while leaving the source snapshot untouched, and HTML paths do not fetch linked assets, execute commands, run templates, run syntax highlighting, or run a math engine. Ordinary structural edits never normalize the whole document. PDF remains outside the current capability set.
...
| Render | deterministic canonical Markdown export; deterministic HTML fragments or standalone documents with explicit raw-HTML, unsafe-URL, tag-filter, metadata policy, and optional source-to-output byte mapping |
