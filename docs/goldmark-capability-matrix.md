...
| Tables | provide table/header/body-row/cell semantics through GFM extension support | preserve table layout/alignment bytes; map editable non-empty cells plus complete body-row physical lines, resolve promoted body row/cell identities, and validate complete row replacement/removal/insertion/movement while leaving delimiter rows and row headers outside row-mutation targets |
