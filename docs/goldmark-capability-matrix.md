...
| Thematic breaks | identify thematic-break block semantics and public source position | use the public start position plus Marksplice-owned line scanning for construction-only canonical `---` proof; do not promote a parsed public mutation kind |
| Blockquotes | identify container semantics and child blocks | prove only the reviewed construction-only single-line/one-paragraph shape from public container position plus child segment; keep general blockquote editing outside the public parsed surface |
...
| Tables | provide semantic table/header/body-row/cell hierarchy plus public per-column alignment values through GFM extension support | preserve table layout/alignment bytes; map editable non-empty cells plus complete body-row physical lines, retain parser-independent private table membership and semantic alignment vectors, resolve promoted body-cell↔row identities, same-table promoted row neighbors, and body-row→promoted-header identities, validate complete row replacement/removal/insertion/movement with alignment-aware survivor proof, and for new documents generate canonical default/left/right/center delimiters while leaving parsed alignment accessors, delimiter rows, and headers outside the reviewed public mutation surface |
...
| Rendering/writing | Goldmark HTML rendering exists but is not Marksplice's GFM source writer | own deterministic GFM writing for reviewed new-document construction; never use Goldmark rendering or whole-document rewriting as the ordinary existing-document edit path |
