...
func TestFootnotesExposeDefinitionsReferencesAndExactSourceOwnership(t *testing.T) {
	t.Parallel()

	source := []byte("before[^b] and again[^a] and again[^a]\n\n[^a]: first line\n\n    second paragraph\n\n[^unused]: unused\n[^b]: bee\n")
	doc := mustParseFootnoteDocument(t, source)
	...
	a := definitions[0]
	assertFootnoteRangeSource(t, doc, a.LabelRange(), "a")
	assertFootnoteRangeSource(t, doc, a.Range(), "[^a]: first line\n\n    second paragraph\n\n")
	...
	assertFootnoteRangeSource(t, doc, aBody[0], "first line")
	assertFootnoteRangeSource(t, doc, aBody[1], "second paragraph")
	...
	assertFootnoteRangeSource(t, doc, body, "bee")
	...
	for index, reference := range references {
		...
		assertFootnoteRangeSource(t, doc, reference.Range(), "[^"+reference.Label()+"]")
		assertFootnoteRangeSource(t, doc, reference.LabelRange(), reference.Label())
	}
...
func TestFootnoteDefinitionsRemainTopLevelAndUnusedDefinitionsAreReadable(t *testing.T) {
	t.Parallel()
...
func TestFootnoteBodyReplacementAndRenameAreSourcePreserving(t *testing.T) {
	t.Parallel()
	...
	bodyDoc := mustParseFootnoteDocument(t, bodySource)
	...
	renamedDoc := mustParseFootnoteDocument(t, renamed)
...
func TestFootnoteMutationsFailClosedOnUnsupportedBodyAndLabelCollision(t *testing.T) {
	t.Parallel()
	...
	multi := mustParseFootnoteDocument(t, []byte("[^n]: first\n\n    second\n"))
...
func TestFootnoteConstructionSupportsDeferredDefinitionsAndTypedReferences(t *testing.T) {
	t.Parallel()
...
func TestLinksInsideFootnotesParticipateInDocumentGraph(t *testing.T) {
	t.Parallel()

	sourceDoc := mustParseFootnoteDocument(t, []byte("note[^n]\n\n[^n]: [target](b.md#part)\n"))
	targetDoc := mustParseFootnoteDocument(t, []byte("# Part\n"))
...
func TestFootnoteSyntaxSupersedesCaretReferenceDefinitions(t *testing.T) {
	t.Parallel()

	doc := mustParseFootnoteDocument(t, []byte("use[^n]\n\n[^n]: /legacy-reference-destination\n"))
...
func TestFootnoteLabelsResolveExactly(t *testing.T) {
	t.Parallel()

	doc := mustParseFootnoteDocument(t, []byte("use[^Note]\n\n[^note]: body\n"))
	definitions := doc.FootnoteDefinitions()
...
func TestFootnoteDefinitionsIntegrateWithStructuralQueries(t *testing.T) {
	...
	doc := mustParseFootnoteDocument(t, []byte("paragraph\n\n[^note]: body\n"))
	definitions := doc.FootnoteDefinitions()
...
func TestFootnoteChangesComposeAcrossIndependentDefinitions(t *testing.T) {
	t.Parallel()
	...
	updated := mustParseFootnoteDocument(t, got)
...
func TestDeferredFootnotesAreRejectedFromQuotedChildBuilders(t *testing.T) {
	t.Parallel()
...
func mustParseFootnoteDocument(t *testing.T, source []byte) *marksplice.Document {
	t.Helper()
...
func assertFootnoteRangeSource(t *testing.T, doc *marksplice.Document, range_ marksplice.Range, want string) {
	t.Helper()
