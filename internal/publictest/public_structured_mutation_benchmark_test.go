...
var changeSink marksplice.ChangeSet

func BenchmarkStructuredMutationPlanning(b *testing.B) {
	benchmarks := []struct {
		name   string
		source []byte
	...
	}{
		{
			...
			setup: func(tb testing.TB, doc *marksplice.Document) func() (marksplice.ChangeSet, error) {
				node := benchmarkNodeOfKind(tb, doc, marksplice.KindFrontMatterField)
		...
		},
		{
			...
			source: []byte("> alpha\n> beta\n\nafter\n"),
			setup: func(tb testing.TB, doc *marksplice.Document) func() (marksplice.ChangeSet, error) {
				node := benchmarkNodeOfKind(tb, doc, marksplice.KindBlockquote)
				return func() (marksplice.ChangeSet, error) {
		...
		},
		{
			...
			setup: func(tb testing.TB, doc *marksplice.Document) func() (marksplice.ChangeSet, error) {
				node := benchmarkNodeOfKind(tb, doc, marksplice.KindInlineLink)
				return func() (marksplice.ChangeSet, error) {
		...
		},
		{
			...
			setup: func(tb testing.TB, doc *marksplice.Document) func() (marksplice.ChangeSet, error) {
				node := benchmarkNodeOfKind(tb, doc, marksplice.KindImage)
				return func() (marksplice.ChangeSet, error) {
...
func benchmarkNodeOfKind(tb testing.TB, doc *marksplice.Document, kind marksplice.Kind) marksplice.Node {
	tb.Helper()
