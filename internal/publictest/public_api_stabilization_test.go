...
func TestWorkspaceUnresolvedReferenceUsesTypedValue(t *testing.T) {
	t.Parallel()
...
func TestImmutablePublicModelsSupportConcurrentReads(t *testing.T) {
	t.Parallel()
	...
	for worker := 0; worker < workers; worker++ {
		...
		go func() {
			...
			for iteration := 0; iteration < iterations; iteration++ {
				if err := exerciseConcurrentReads(docA, graph, knowledge, report, change, sourceA); err != nil {
					errors <- err
					return
				}
			}
...
func exerciseConcurrentReads(document *marksplice.Document, graph *marksplice.DocumentGraph, knowledge *marksplice.KnowledgeIndex, report *marksplice.WorkspaceReport, change marksplice.ChangeSet, source []byte) error {
