...
func TestPathologicalInputsRemainOperable(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "malformed inline link storm", source: repeatedSource(4096, func(i int) string { return fmt.Sprintf("[broken-%d]( ", i) })},
		{name: "many duplicate headings", source: repeatedSource(4096, func(i int) string { return "# Same heading\n\n" })},
		{name: "many local links", source: repeatedSource(4096, func(i int) string { return fmt.Sprintf("[target-%d](#target-%d) ", i, i) })},
		{name: "many footnotes and references", source: repeatedSource(2048, func(i int) string { return fmt.Sprintf("[^note-%d]: body %d\n\nref[^note-%d]\n\n", i, i, i) })},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assertReadSurfaces(t, document, tt.source)
		})
	}
}

func FuzzPublicReadSurfacesRemainSourceBound(f *testing.F) {
...
func FuzzNoopMutationsPreserveExactSource(f *testing.F) {
	for _, seed := range [][]byte{
		[]byte("# Title\n\nparagraph\n"),
		[]byte("- item\n\n```go\nbody\n```\n"),
	...
	f.Fuzz(func(t *testing.T, source []byte) {
		...
		for _, match := range matches {
			...
			change, supported, err := noopChange(document, match.Node(), payload)
			if err != nil {
...
func noopChange(document *marksplice.Document, node marksplice.Node, payload []byte) (marksplice.ChangeSet, bool, error) {
	switch node.Kind() {
...
func repeatedSource(count int, line func(int) string) []byte {
