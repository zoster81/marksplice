...
var commonMarkProfileDivergences = map[int]string{
...
var gFMProfileDivergences = map[int]string{
...
func TestPublishedCommonMarkHTMLFullProfileContract(t *testing.T) {
	...
	seen := make(map[int]bool, len(commonMarkProfileDivergences))
	for _, case_ := range cases {
		if _, divergent := commonMarkProfileDivergences[case_.Number]; divergent {
	...
	assertProfileDivergencesPresent(t, "CommonMark", commonMarkProfileDivergences, seen)
}

func TestPublishedGFMHTMLFullProfileContract(t *testing.T) {
	...
	seen := make(map[int]bool, len(gFMProfileDivergences))
	for _, case_ := range cases {
		if _, divergent := gFMProfileDivergences[case_.Number]; divergent {
			seen[case_.Number] = true
			continue
		}
		document, err := marksplice.Parse([]byte(case_.Markdown))
	...
	assertProfileDivergencesPresent(t, "GFM", gFMProfileDivergences, seen)
}

func assertProfileDivergencesPresent(t *testing.T, profile string, divergences map[int]string, seen map[int]bool) {
	t.Helper()
