...
func TestFencedBlocksExposeExactContainerInfoLanguageAndBodyRanges(t *testing.T) {
	t.Parallel()

	source := []byte("before\n\n  ~~~~  mermaid diagram  \n  graph TD\n  A-->B\n ~~~~~~   \n\nafter\n")
	doc := mustParseFencedBlockDocument(t, source)
	...
	assertFencedBlockRangeSource(t, doc, block.OpeningFenceRange(), "~~~~")
	assertFencedBlockRangeSource(t, doc, closingRange, "~~~~~~")
	assertFencedBlockRangeSource(t, doc, block.Range(), "  ~~~~  mermaid diagram  \n  graph TD\n  A-->B\n ~~~~~~   \n")
	...
	assertFencedBlockRangeSource(t, doc, infoRange, "mermaid diagram")
	...
	assertFencedBlockRangeSource(t, doc, body[0], "graph TD")
	assertFencedBlockRangeSource(t, doc, body[1], "A-->B")
	original := body[0]
...
func TestEmptyFencedBlockIsReadableAndConstructible(t *testing.T) {
	t.Parallel()
	...
	built := mustParseFencedBlockDocument(t, markdown).FencedBlocks()
	if len(built) != 1 {
		t.Fatalf("built len(FencedBlocks()) = %d, want 1", len(built))
	}
	if ranges, ok := mustParseFencedBlockDocument(t, markdown).FencedBlockContentRanges(built[0].ID()); !ok || len(ranges) != 0 {
		t.Fatalf("built content ranges = %v/%v, want empty/true", ranges, ok)
	}
}

func TestUnclosedContiguousFenceKeepsLegacyReplacementContract(t *testing.T) {
	t.Parallel()
	...
	assertFencedBlockRangeSource(t, doc, block.Range(), string(source))
	...
	assertFencedBlockRangeSource(t, doc, legacy.Range(), "{\"type\":\"Point\"}")
...
func TestHistoricalFencedCodeRangeAndClosedReplacementRemainStable(t *testing.T) {
	t.Parallel()
	...
	assertFencedBlockRangeSource(t, doc, legacy.Range(), "line one\nline two")
...
func TestFencedBlocksStayTopLevelAndFailClosedOnUnknownIDs(t *testing.T) {
	t.Parallel()
...
func TestEmptyReplacementStillFailsClosedForLegacyFencedCode(t *testing.T) {
	t.Parallel()
...
func mustParseFencedBlockDocument(t *testing.T, source []byte) *marksplice.Document {
	t.Helper()
...
func assertFencedBlockRangeSource(t *testing.T, doc *marksplice.Document, range_ marksplice.Range, want string) {
	t.Helper()
