...
func TestMathExpressionsExposeDollarBacktickBlockAndFencedForms(t *testing.T) {
	t.Parallel()

	source := []byte("inline $x+1$ and $`a*b`$\n\n$$x^2$$\n\n```math\na+b\n```\n")
	doc := mustParseMathDocument(t, source)
	...
	for index, expression := range expressions {
		...
		assertMathRangeSource(t, doc, expression.Range(), wantSyntax[index])
		...
		assertMathRangeSource(t, doc, payloadRanges[0], wantPayload[index])
...
func TestMathRecognitionFailsClosedAroundMarkdownOwnedAndMalformedSource(t *testing.T) {
	t.Parallel()
	...
	assertMathRangeSource(t, doc, payload, "x\\$y")
}

func TestMathPayloadReplacementPreservesSourceForm(t *testing.T) {
	t.Parallel()
...
func TestMathCompositionRejectsTwoEditsInSameParagraphAggregate(t *testing.T) {
	t.Parallel()

	doc := mustParseMathDocument(t, []byte("$x$ and $`y`$\n"))
	expressions := doc.MathExpressions()
...
func TestMathConstructionAndQueries(t *testing.T) {
	t.Parallel()
	...
	doc := mustParseMathDocument(t, markdown)
...
func mustParseMathDocument(t *testing.T, source []byte) *marksplice.Document {
	t.Helper()
...
func assertMathRangeSource(t *testing.T, doc *marksplice.Document, range_ marksplice.Range, want string) {
	t.Helper()
