...
// TableCellMapping binds one non-empty semantic GFM table cell to its raw cell span.
...
// MapTableCell verifies a semantic table-cell range against one physical GFM table row.
func MapTableCell(input []byte, content Range, column int) (TableCellMapping, error) {
	...
	lineStart := physicalLineStart(input, content.Start)
	lineEnd := physicalLineEnd(input, content.End)
	spans := tableCellSpans(input[lineStart:lineEnd])
	if column >= len(spans) {
		return TableCellMapping{}, fmt.Errorf("%w: column %d is outside %d mapped cells", ErrUnsupportedTableCellShape, column, len(spans))
	}

	raw := spans[column]
	trimmed := trimHorizontalSpaceRange(input[lineStart:lineEnd], raw)
	mappedContent := Range{Start: lineStart + trimmed.Start, End: lineStart + trimmed.End}
	if mappedContent != content {
		return TableCellMapping{}, fmt.Errorf("%w: semantic content [%d,%d) does not match mapped content [%d,%d)", ErrUnsupportedTableCellShape, content.Start, content.End, mappedContent.Start, mappedContent.End)
	}

	return TableCellMapping{
