...
func TestM123CanonicalMarkdownRealWorldCorpus(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(realWorldBenchmarkName(file.relative), func(t *testing.T) {
			...
			originalFacts := collectM123CanonicalSemanticFacts(t, backend, file.source)

			var first bytes.Buffer
			if panicValue, err := renderM123Canonical(&first, file.source, backend); panicValue != nil {
			...
			canonicalFacts := collectM123CanonicalSemanticFacts(t, backend, first.Bytes())
			if !reflect.DeepEqual(canonicalFacts, originalFacts) {
				t.Fatalf("canonical rendering changed semantic facts: %s", firstM123CanonicalSemanticDifference(originalFacts, canonicalFacts))
			}

			var second bytes.Buffer
			if panicValue, err := renderM123Canonical(&second, first.Bytes(), backend); panicValue != nil {
...
func BenchmarkM123CanonicalMarkdownRealWorldCorpus(b *testing.B) {
...
func BenchmarkM123CanonicalMarkdownLargestFiles(b *testing.B) {
...
func renderM123Canonical(writer io.Writer, source []byte, backend rendermarkdown.Backend) (panicValue any, err error) {
	defer func() {
		panicValue = recover()
	}()
...
func collectM123CanonicalSemanticFacts(t *testing.T, backend rendermarkdown.Backend, source []byte) []canonicalSemanticFact {
	t.Helper()
	...
	if err := backend.WalkSemantic(source, func(event parser.SemanticEvent) error {
		...
		fact := m123CanonicalSemanticFactForEvent(event)
...
func m123CanonicalSemanticFactForEvent(event parser.SemanticEvent) canonicalSemanticFact {
	value := event.Value
	switch event.Kind {
	case parser.SemanticFrontMatter, parser.SemanticRawHTML, parser.SemanticCodeBlock:
		value = normalizeM123CanonicalLineEndings(value)
	case parser.SemanticHTMLBlock:
		value = strings.TrimSuffix(normalizeM123CanonicalLineEndings(value), "\n")
	}
	label := event.Label
	...
	return canonicalSemanticFact{
		...
		title:       decodeM123CanonicalMarkdownSyntaxString(event.Title),
...
func firstM123CanonicalSemanticDifference(before, after []canonicalSemanticFact) string {
	limit := len(before)
	if len(after) < limit {
		limit = len(after)
	}
...
func normalizeM123CanonicalLineEndings(value string) string {
	value = strings.ReplaceAll(value, "\r\n", "\n")
	return strings.ReplaceAll(value, "\r", "\n")
}

func decodeM123CanonicalMarkdownSyntaxString(value string) string {
	var output strings.Builder
	for position := 0; position < len(value); {
		if value[position] == '\\' && position+1 < len(value) && isM123CanonicalASCIIPunctuation(value[position+1]) {
...
func isM123CanonicalASCIIPunctuation(value byte) bool {
