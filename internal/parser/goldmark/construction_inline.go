...
func ValidateConstructionInlineHierarchy(source []byte, expected []parser.ConstructionInlineExpectation) error {
	if len(expected) == 0 {
		return nil
	}
	root := newMarkdown().Parser().Parse(text.NewReader(normalizeIsolatedCR(source)))
	if root.ChildCount() != 1 {
		return fmt.Errorf("expected one paragraph for typed inline hierarchy")
	}
	paragraph, ok := root.FirstChild().(*ast.Paragraph)
	if !ok {
		return fmt.Errorf("typed inline hierarchy is not contained by a paragraph")
	}
	...
	if err := validateConstructionInlineRootChildren(paragraph, rootChildren, actual); err != nil {
		return err
	}
	for index := range expected {
		if err := validateConstructionInlineChildren(actual[index], index, childrenByParent[index], actual); err != nil {
			return err
		}
	}
	return nil
...
func matchConstructionInlineNodes(paragraph *ast.Paragraph, expected []parser.ConstructionInlineExpectation) ([]ast.Node, error) {
	...
	if err := ast.Walk(paragraph, func(node ast.Node, entering bool) (ast.WalkStatus, error) {
		if !entering || node == paragraph || !constructionInlineSemanticNode(node) {
...
func validateConstructionInlineRootChildren(paragraph *ast.Paragraph, wantChildren []int, actual []ast.Node) error {
	gotChildren := make([]ast.Node, 0, len(wantChildren))
	for child := paragraph.FirstChild(); child != nil; child = child.NextSibling() {
		if constructionInlineSemanticNode(child) {
...
func validateConstructionInlineChildren(node ast.Node, parent int, wantChildren []int, actual []ast.Node) error {
	gotChildren := make([]ast.Node, 0, len(wantChildren))
...
func constructionInlineSemanticNode(node ast.Node) bool {
	switch node.(type) {
